...
/// 可以先在左邊、上面，準備 2 條長條的陣列，若 matrix[i][j]==0 就把 left[i] 和 up[j] 打勾勾
...
public:
    void setZeroes(vector<vector<int>>& matrix) {
        ...
        vector<int> left(M), up(N); /// 左邊長條、上面長條的 C++ 陣列
        for(int i=0; i<M; i++){ /// 左手 i 範圍是 M
            for(int j=0; j<N; j++){ /// 右手 j 範圍是 N
                if(matrix[i][j]==0){ /// 遇到有 0，就在左邊長邊、上面長條打勾勾
                    left[i]=7; /// 打勾勾
                    up[j]=7; /// 打勾勾
                }
            }
