...
## 找最長 subsequence 裡面「相鄰的項」groups 不相同、words[i] words[j]長度相同、只差 1 字母

class Solution:
    def getWordsInLongestSubsequence(self, words: List[str], groups: List[int]) -> List[str]:
        n = len(words) ## words 有幾個數
        prev = [-1]*n ## 用來 ｢指到｣ 前一項，也就是 words[i] 前一項 prev[i]
        @cache ## 利用 cache 加速
        def hammingOne(i, j): ## 判斷 2 字串的距離「剛好是 1」的狀況
            if len(words[i]) != len(words[j]): return False ## 長度不同，失敗
            diff = 0 ## 兩字串的距離
            for k in range(len(words[i])): ## 逐字母比較
                if words[i][k] != words[j][k]: diff += 1 ## 不同的字母有幾個
            return diff == 1 ## 剛好「距離是 1」
        @cache ## 可用 DP 「函式呼叫函式」
        def helper(k): ## 「使用 words[k] 結尾」 的最長 subsequence
            if k == 0: return 1 ## 最基礎的 case 就 words[0] 這 1 個字
            ans = 1 ## 最長的長度，至少有 words[k] 本身 1 個
