...
# edges 紀錄「node 向外走到哪裡」 ex. edges[node1] 是 node1 下一步會到的 node
# 能從 node1 和 node2 出發，找出「最快相遇」的點 (index 要最小的)

class Solution:
    def closestMeetingNode(self, edges: List[int], node1: int, node2: int) -> int:
        p1, p2 = node1, node2   # 從 node1 和 node2 出發
        set1, set2 = set(), set()   # 紀錄「分別走過」的 nodes

        while True:
            set1.add(p1)    # 走過
            set2.add(p2)    # 走過
            if p1 in set2 and p2 in set1:   # 同時走到「對方」的領地，就算「相遇」
                return min(p1, p2)  # (index 要最小的)，當答案
