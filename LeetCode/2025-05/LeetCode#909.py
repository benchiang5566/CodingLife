...
# 蛇梯 每個格子編號「繞來繞去」同時有許多「直達車」，採到時「瞬間」換位置
# 每次可以「增加 1~6 格」，甚麼時候可以到「最後 1 格」。看起來 BFS 可解
...
class Solution:
    def snakesAndLadders(self, board: List[List[int]]) -> int:
        ...
        for k in range(N*N):    # 一維陣列迴圈
            i, j = k // N, k%N  # 傳統變二維陣列的 i, j 座標
            if i%2==1: j = N-1-j    # 在奇數層，會「反過來走」變成「右到左」
            i = N-1-i   # 把「上下」倒過來，變成從下到上走
            table[k+1] = board[i][j]    # 把「二維陣列」的蛇梯，換算成「一維陣列」，從 1 開始 
        # 接下來，使用 BFS 的方式，試著走走看，看何時能走到 table[N + N] 的這格
        queue = deque() # 用 queue 進行 BFS
        queue.append( (0,1) )   # 第 0 步，站在 table[1] 的位置
        visited = set() # 走過的格子，不要再走
        visited.add(1)  # 出發點算有走過

        while queue:    # 進行 BFS
            step, k = queue.popleft()   # 第 step 走，能走到第 k 格
            if k == N*N: return step    # 到達終點
